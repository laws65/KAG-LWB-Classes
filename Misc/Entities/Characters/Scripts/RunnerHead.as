// added default head for new classes.
...
int getHeadFrame(CBlob@ blob, int headIndex, bool default_pack)
{
	...
	if (default_pack && (headIndex == 255 || headIndex < NUM_UNIQUEHEADS))
	{
		...
		if (!holidayhead)
		{
			...
			else if (config == "medic")
			{
				headIndex = NUM_UNIQUEHEADS + 12;
			}
			else if (config == "spearman")
			{
				headIndex = NUM_UNIQUEHEADS + 21;
			}
			else if (config == "assassin")
			{
				headIndex = NUM_UNIQUEHEADS + 28;
			}
			else if (config == "crossbowman")
			{
				headIndex = NUM_UNIQUEHEADS + 15;
			}
