	...
	if (caller !is null && caller.isMyPlayer() && classes !is null)
	{
		if (this.hasTag("all_classes_loaded"))
		{
			...
			for (uint i = 0 ; i < classes.length; i++)
			{
				...
				if (name == "builder" || name == "rockthrower" || name == "medic")
					supportclasses.push_back(classes[i]);
				else if (name == "knight" || name == "spearman" || name == "assassin")
					meleeclasses.push_back(classes[i]);
				else if (name == "archer" || name == "crossbowman" || name == "musketman")
			...
			}
			{
				...
				if (menu !is null)
				{
					for (uint i = 0 ; i < supportclasses.length; i++)
					{
						...
						CBitStream params;
						write_classchange(params, caller.getNetworkID(), pclass.configFilename);
			...
			{
				CGridMenu@ menu = CreateGridMenu(caller.getScreenPos() + Vec2f(24.0f, caller.getRadius() * 1.0f + 16.0f), this, Vec2f(rangedclasses.length * CLASS_BUTTON_SIZE, CLASS_BUTTON_SIZE), "Ranged classes");
				if (menu !is null)
				{
			...
			CGridMenu@ menu = CreateGridMenu(caller.getScreenPos() + Vec2f(24.0f, caller.getRadius() * 1.0f + 48.0f), this, Vec2f(3 * CLASS_BUTTON_SIZE, 3 * CLASS_BUTTON_SIZE), getTranslatedString("Swap class"));
			if (menu !is null)
			{
