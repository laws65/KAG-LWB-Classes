...
void MakeTradeMenu(CBlob@ trader)
{
	...
	s32 cost_spears = cfg.read_s32("cost_spears", 10);
	s32 cost_firespears = cfg.read_s32("cost_firespears", 30);
	s32 cost_poisonspears = cfg.read_s32("cost_poisonspears", 50);
	s32 cost_smokeball = cfg.read_s32("cost_smokeball", 50);
	...
	s32 cost_poisonarrows = cfg.read_s32("cost_poisonarrows", 40);
	s32 cost_bullets = cfg.read_s32("cost_bullets", 10);
	s32 cost_barricades = cfg.read_s32("cost_barricades", 50);

	s32 cost_boulder = cfg.read_s32("cost_boulder", 10);
	...
	s32 cost_wood = cfg.read_s32("cost_wood", 50);
	s32 cost_stone = cfg.read_s32("cost_stone", 10);
	s32 cost_medkit = cfg.read_s32("cost_medkit", 10);
	s32 cost_waterjar = cfg.read_s32("cost_waterjar", 30);
	s32 cost_poisonjar = cfg.read_s32("cost_poisonjar", 30);
	s32 cost_acidjar = cfg.read_s32("cost_acidjar", 30);
	s32 cost_catapult = cfg.read_s32("cost_catapult", -1);
	s32 cost_ballista = cfg.read_s32("cost_ballista", -1);

	s32 menu_width = cfg.read_s32("trade_menu_width", 6);
	...
	addItemForCoin(trader, "Spears", cost_spears, true, "$mat_spears$", "mat_spears", "Spare Spears for Spearman. Throw them to enemies.");
	addItemForCoin(trader, "Fire Spear", cost_firespears, true, "$mat_firespears$", "mat_firespears", "Fire Spear for Spearman. Make spear attacking or thrown spear ignitable once.");
	addItemForCoin(trader, "Poison Spears", cost_poisonspears, true, "$mat_poisonspears$", "mat_poisonspears", "Poison Spears for Spearman. Make spear attacking or thrown spear poisonable twice.");
	addItemForCoin(trader, "Smoke Ball", cost_smokeball, true, "$mat_smokeball$", "mat_smokeball", "Smoke Ball for Assassin. Can stun nearly enemies.");
	...
	addItemForCoin(trader, "Poison Arrows", cost_poisonarrows, true, "$mat_poisonarrows$", "mat_poisonarrows", "Poison arrows for Archer and Crossbowman.");
	addItemForCoin(trader, "Bullets", cost_bullets, true, "$mat_bullets$", "mat_bullets", "Lead ball and gunpowder in a paper for Musketman.");
	addItemForCoin(trader, "Barricade ", cost_barricades, true, "$mat_barricades$", "mat_barricades", "Ballicade frames for Musketman.");
	//utility stuff
	addItemForCoin(trader, "Wood", cost_wood, true, "$mat_wood$", "mat_wood", Descriptions::wood);
	addItemForCoin(trader, "Stone", cost_stone, true, "$mat_stone$", "mat_stone", Descriptions::stone);
	addItemForCoin(trader, "Med Kit", cost_medkit, true, "$mat_medkits$", "mat_medkits", "Med kit for Medic. Can be used 10 times.");
	addItemForCoin(trader, "Water in a Jar", cost_waterjar, true, "$mat_waterjar$", "mat_waterjar", "Water for Medic Spray.");
	addItemForCoin(trader, "Poison in a Jar", cost_poisonjar, true, "$mat_poisonjar$", "mat_poisonjar", "Poison for Medic Spray.");
	addItemForCoin(trader, "Acid in a Jar", cost_acidjar, true, "$mat_acidjar$", "mat_acidjar", "Acid for Medic Spray.\nCan damage blocks and enemies.");
