...
void onInit(CBlob@ this)
{
	...
	SetHelp(this, "help self action2", "rockthrower", getTranslatedString("$Help_Hammer$Dig/Chop  $KEY_HOLD$$RMB$"), "", 3);
...
void onTick(CBlob@ this)
{
	...
	bool throwAnim = sprite.isAnimation("throw");

	// slow down walking
	bool slow = this.isKeyPressed(key_action2);
	if (!slow && throwAnim)
	{
		slow = true;
		CBlob@ maybeDrill = this.getCarriedBlob();
		if(maybeDrill !is null && maybeDrill.getName() == "drill") slow = false;
	}
	if (slow)
	{
	...
	{
		Pickaxe(this);
		if(slow && throwAnim) Throw(this, sprite);// no throw when drill
		RockthrowerInfo@ rter;
		if (!this.get("rockthrowerInfo", @rter))
		{
			return;
		}
		if (rter.boulderTimer > 0) rter.boulderTimer--;


		if(this.getCarriedBlob() is null && rter.action == Action::ladder && this.isKeyJustPressed(key_action1))// reload ladder
		...
		else if (this.isKeyJustPressed(key_action3))
		{
			...
			{
				if(rter.boulderTimer == 0 && hasBoulderStones(this))
				{
					this.SendCommand(this.getCommandID("constructboulder"));
					rter.boulderTimer = 150;
				}
				else
				{
					Sound::Play("/NoAmmo");
			...
			else if (carried is null || carried.getName() != "ladder")// TODO: use hasTag("temp blob"), the tag isn't synced on first build... why
			{
...
bool RecdHitCommand(CBlob@ this, CBitStream@ params)
{
	...
	else
	{
		...
		if (blob !is null)
		{
			...
			if (getNet().isServer())
			{
				this.server_Hit(blob, tilepos, attackVel, attack_power, Hitters::hammer, teamHurt);
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	else if (cmd == this.getCommandID("throwrock"))
	{
		Vec2f rockPos;
		if (!params.saferead_Vec2f(rockPos)) return;
		Vec2f rockVel;
		if (!params.saferead_Vec2f(rockVel)) return;

		this.getSprite().PlaySound("/ArgLong");
		if (!getNet().isServer() || !hasStones(this))
			return;
	...
	else if (cmd == this.getCommandID("constructboulder"))
	{
		this.getSprite().PlaySound("/ConstructShort");
		if (!getNet().isServer() || !hasBoulderStones(this))
			return;
...
void Throw(CBlob@ this, CSprite @sprite)
{
	bool justCheck = !sprite.isFrameIndex(hit_frame);
	bool adjusttime = sprite.getFrameIndex() < hit_frame - 1;
	if (!adjusttime && !justCheck)
	{
		if (hasStones(this))
		{
			Vec2f rockPos = this.getPosition() + Vec2f(0.0f, -2.0f);
			Vec2f rockVel = this.getAimPos() + Vec2f(0.0f, -2.0f) - rockPos;
			CBitStream params;
			params.write_Vec2f(rockPos);
			params.write_Vec2f(rockVel);
			this.SendCommand(this.getCommandID("throwrock"), params);
		}
		else
		{
			if (this.isMyPlayer())
...
void onAddToInventory(CBlob@ this, CBlob@ blob)
{
	...
	{
		SetHelp(this, "help inventory", "rockthrower", "$Help_Action1$$Swap$$Help_Action2$           $KEY_TAP$$KEY_F$", "", 3);
	}
}
